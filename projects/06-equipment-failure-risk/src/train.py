"""
Train and evaluate the failure-risk classifier with a temporal split
(train on the past, test on the most recent truck-days). At a ~2.2%
unplanned-failure rate, accuracy is close to useless as a metric
(predicting "never fails" already scores ~97.8%), so average precision
(PR-AUC) is the headline number here instead of ROC-AUC, and the
decision threshold is picked from actual downtime-cost and
inspection-cost assumptions rather than a default 0.5 cutoff, the same
cost-based approach project 01 uses for delinquency and project 05
uses for fraud.
...
SCHEDULED_INSPECTION_COST_USD = 4_000.0


def find_cost_optimal_threshold(y_true, y_prob, downtime_costs):
    ...
    for t in thresholds:
        ...
        cost = (downtime_costs[fn_mask] + UNPLANNED_FAILURE_REPAIR_PREMIUM_USD).sum() + fp_mask.sum() * SCHEDULED_INSPECTION_COST_USD
        costs.append(cost)
...
def main():
    df = pd.read_csv(BASE / "data" / "truck_telemetry.csv", parse_dates=["date"])
    train_df, val_df, test_df = temporal_split(df)
    print(f"Train: {len(train_df):,} | Val: {len(val_df):,} | Test: {len(test_df):,}")
    SOURCE = f"Source: synthetic mining fleet telemetry · held-out test set · n = {len(test_df):,} truck-days"
    ...
    print(f"Train failure rate: {y_train.mean():.3%} | Test failure rate: {y_test.mean():.3%}")
    ...
    # asymmetry between a missed failure and a false alarm lives anyway.
    ...
    # --- The accuracy paradox: a trivial "always healthy" classifier ---
    ...
    style_ax(ax, title="Precision-recall curve",
             subtitle="Held-out test set, 2.2% unplanned-failure base rate",
    ...
    metrics = {
        ...
        "train_failure_rate": round(float(y_train.mean()), 5),
        "test_failure_rate": round(float(y_test.mean()), 5),
        "logistic_baseline_pr_auc": round(float(logit_ap), 4),
        "gbm_test_roc_auc": round(float(auc), 4),
        "gbm_test_pr_auc": round(float(ap), 4),
        "naive_always_healthy_accuracy": round(float(naive_accuracy), 4),
        ...
        "n_failures_test": n_failures_test,
