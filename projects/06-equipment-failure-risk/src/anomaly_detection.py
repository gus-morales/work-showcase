"""
How much does having failure labels actually buy you? Isolation Forest
scores each truck-day's "unusualness" using only the feature values, no
labels at all, then that ranking is checked against the same held-out
failure outcomes the supervised GBM in train.py was evaluated against.
This is the scenario a maintenance team faces before enough confirmed
failure events exist to train a supervised model, or when a new fleet
or truck class doesn't have failure history to learn from yet.
...
def fit_isolation_forest(X_train):
    """Isolation Forest never sees failure_within_7d; it isolates points
    that are easy to separate from the rest with few random splits, the
...
def anomaly_scores(iso, X):
    ...
    the supervised model's failure probability already uses."""
...
def main():
    ...
    source_note = f"Source: synthetic mining fleet telemetry · held-out test set · n = {len(test_df):,} truck-days"
