...
The single biggest fact about this dataset is the imbalance itself: 1,269 fraud cases against 88,955 genuine transactions, a 70:1 ratio (1.407% fraud rate). Plotted on a linear scale rather than log, the fraud bar is nearly invisible next to genuine, which is the actual shape of the problem every metric choice and threshold decision below has to work around (Figure 1).

![Class imbalance](reports/figures/class_imbalance.png)

*Figure 1. Transaction count by class, genuine vs. fraud.*

An unrecognized device is already one of the strongest single signals before any model gets trained: 7.20% of transactions from a new/unrecognized device are fraudulent, against 1.08% from a recognized one (Figure 2).

![Fraud rate by device recognition](reports/figures/fraud_rate_by_device.png)

*Figure 2. Fraud rate by device recognition.*

Fraud rate also climbs with recent transaction velocity: 1.35% with no prior transaction in the last hour, 1.77% with one, and 3.42% with two or more (Figure 3).

![Fraud rate by transaction velocity](reports/figures/fraud_rate_by_velocity.png)

*Figure 3. Fraud rate by transactions in the 1-hour window before this one.*
...
The GBM ranks transactions well above random despite the skew (Figure 4); sweeping the decision threshold against expected cost shows why the optimal cutoff (0.03) sits so far below the usual 0.5 default (Figure 5).

![Precision-recall curve](reports/figures/pr_curve.png)

*Figure 4. Precision-recall curve on the held-out test set, GBM vs. the 1.07% base rate.*
...
On the same held-out set, it clears random ranking by a real margin (2.7x the base rate), but falls well short of the supervised model (11.0x), a 4.2x gap (Figure 8). Having labels is clearly worth a lot. But an unsupervised score before those labels exist is still a useful signal in the meantime.
